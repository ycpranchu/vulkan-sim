...
ray_coherence_engine::ray_coherence_engine(unsigned sid,
                                           struct ray_coherence_config config,
...
void ray_coherence_engine::insert(warp_inst_t inst) {
  ...
  for (unsigned i = 0; i < m_config.warp_size; i++) {
    ...
    if (m_ray_pool.find(hash) == m_ray_pool.end()) {
      COHERENCE_DPRINTF(
          "Shader %d: New coherence packet created for hash 0x%x\n", m_sid,
          hash);
...
bool ray_coherence_engine::is_stalled() {
  ...
  for (auto i = m_scheduled_packets.cbegin(); i != m_scheduled_packets.cend();
       i++) {
...
bool ray_coherence_engine::is_stalled(const coherence_packet packet) {
  ...
  for (auto it = packet.cbegin(); it != packet.cend(); it++) {
    coherence_ray ray = *it;
    if (!ray.empty()) {
      if (ray.next_status() == RT_MEM_UNMARKED && ray.latency_delay == 0)
        return false;
    }
  }
  // Stalled
...
unsigned ray_coherence_engine::schedule_next_warp() {
  ...
  COHERENCE_DPRINTF("Shader %d: Scheduling next access (packet %d ", m_sid,
                    m_schedule_packet_id);
  ...
  for (auto it = selected_packet.cbegin(); it != selected_packet.cend(); it++) {
    ...
    if (!ray.empty()) {
      ...
      if (ray.next_status() != RT_MEM_AWAITING && ray.latency_delay == 0) {
        addr_size_pair request =
            addr_size_pair(ray.next_access().address, ray.next_access().size);
  ...
  COHERENCE_DPRINTF("addr 0x%x size %d ", next_request.first,
                    next_request.second);
  ...
  for (auto it = selected_packet.cbegin(); it != selected_packet.cend(); it++) {
    ...
    if (!ray.empty() && ray.latency_delay == 0) {
      if (ray.next_access().address == next_request.first &&
          ray.next_access().size == next_request.second) {
...
RTMemoryTransactionRecord ray_coherence_engine::get_next_access() {
  ...
  for (coherence_ray &ray : selected_packet) {
    if (!ray.empty()) {
      if (ray.next_addr() == m_active_record.address &&
        ...
        COHERENCE_DPRINTF(
            "Shader %d: Mark mem awaiting for warp %d thread %d\n", m_sid,
            ray.origin_warp_uid, ray.origin_thread_id);
      }
    }
  }
  ...
  COHERENCE_DPRINTF(
      "Shader %d: Inserting MSHR entry for packet %d at addr 0x%x\n", m_sid,
      m_schedule_packet_id, m_active_record.address);
  m_request_mshr[m_active_record.address].insert(m_schedule_packet_id);

  // Create MSHR for chunks
  if (m_active_record.size > 32) {
    COHERENCE_DPRINTF(
        "Shader %d: Memory request > 32B. Inserting MSHR entries\n", m_sid);
    // Create the memory chunks and push to mem_access_q
    for (unsigned i = 1; i < ((m_active_record.size + 31) / 32); i++) {
      if (m_request_mshr.find(m_active_record.address + (i * 32)) ==
          m_request_mshr.end()) {
        std::set<unsigned> outstanding_packets;
        m_request_mshr[m_active_record.address + (i * 32)] =
            outstanding_packets;
      }
      COHERENCE_DPRINTF(
          "Shader %d: Inserting MSHR entry for packet %d at addr 0x%x\n", m_sid,
          m_schedule_packet_id, m_active_record.address + (i * 32));
      m_request_mshr[m_active_record.address + (i * 32)].insert(
          m_schedule_packet_id);
    }
  }
...
void ray_coherence_engine::undo_access(new_addr_type addr) {
  ...
  for (coherence_ray &ray : selected_packet) {
    if (!ray.empty()) {
      if (ray.next_addr() == m_active_record.address &&
        ...
        COHERENCE_DPRINTF(
            "Shader %d: Undo mem awaiting for warp &d thread %d\n", m_sid,
            ray.origin_warp_uid, ray.origin_thread_id);
      }
    }
  }
  ...
  COHERENCE_DPRINTF(
      "Shader %d: Undoing MSHR entry for packet %d at addr 0x%x\n", m_sid,
      m_schedule_packet_id, addr);
}

void ray_coherence_engine::process_response(
    mem_fetch *mf, std::map<unsigned, warp_inst_t *> &m_current_warps,
  ...
  if (m_request_mshr.find(uncoalesced_addr) != m_request_mshr.end()) {
    ...
    COHERENCE_DPRINTF(
        "Shader %d: Found %d MSHR ray coherency packets for addr 0x%x\n", m_sid,
        packets.size(), uncoalesced_addr);
    ...
    for (unsigned p : packets) {
      ...
      for (coherence_ray &ray : packet) {
        if (!ray.empty() && ray.latency_delay == 0) {
          ...
          assert(pipe_reg->get_uid() == warp_uid ||
                 m_current_warps.find(warp_uid) != m_current_warps.end());
          if (uncoalesced_base_addr == ray.next_addr()) {
            COHERENCE_DPRINTF(
                "Shader %d: Ray coherency packet includes warp %d thread %d\n",
                m_sid, warp_uid, thread_id);
            bool mem_record_done =
                (!pipe_reg->empty() && pipe_reg->get_uid() == warp_uid)
                    ? pipe_reg->process_returned_mem_access(mf, thread_id)
                    : m_current_warps[warp_uid]->process_returned_mem_access(
                          mf, thread_id);
            ...
            if (mem_record_done) {
              ...
              ray.latency_delay =
                  (!pipe_reg->empty() && pipe_reg->get_uid() == warp_uid)
                      ...
                      : m_current_warps[warp_uid]->get_thread_latency(
                            thread_id);
            }
          }
        }
...
// `theta_bits` is used for theta, `theta_bits` + 1 is used for phi, for a total
// of 2 * `theta_bits` + 1 bits
...
uint64_t ray_coherence_engine::hash_origin_grid(const float3 &o,
                                                uint32_t num_bits) {
  uint32_t grid_size = 1 << num_bits;

  uint64_t hash_o_x =
      clamp((o.x - world_min.x) / (world_max.x - world_min.x) * grid_size, 0.f,
            (float)grid_size - 1);
  uint64_t hash_o_y =
      clamp((o.y - world_min.y) / (world_max.y - world_min.y) * grid_size, 0.f,
            (float)grid_size - 1);
  uint64_t hash_o_z =
      clamp((o.z - world_min.z) / (world_max.z - world_min.z) * grid_size, 0.f,
            (float)grid_size - 1);
...
ray_hash ray_coherence_engine::hash_two_point(const Ray &ray) {
  ...
  float3 est_target =
      ray.get_origin() + m_config.hash_two_point_est_length_ratio *
                             max_extent_length * ray.get_direction();
...
unsigned long long ray_coherence_engine::compute_index(
    ray_hash hash, unsigned num_bits) const {
...
void ray_coherence_engine::print_full(FILE *fout) {
  ...
  for (unsigned i = 0; i < m_config.max_packets; i++) {
    if (i == m_schedule_packet_id) fprintf(fout, "*");
    fprintf(fout, "[%d] (%s)\n", i,
            is_stalled(m_scheduled_packets[i]) ? "s" : " ");
...
void ray_coherence_engine::print_stats(FILE *fout) { m_stats->print(fout); }
