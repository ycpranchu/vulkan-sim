...
typedef struct Vulkan_RT_thread_data {
  ...
  variable_decleration_entry* get_variable_decleration_entry(
      nir_variable_mode type, std::string name, uint32_t size) {
    if (type == nir_var_ray_hit_attrib) return get_hitAttribute();
  ...
  void set_hitAttribute(float3 barycentric, const ptx_instruction* pI,
                        ptx_thread_info* thread) {
    variable_decleration_entry* hitAttribute = get_hitAttribute();
    float* address;
    if (hitAttribute == NULL) {
      address = (float*)add_variable_decleration_entry(nir_var_ray_hit_attrib,
                                                       "attribs", 12);
    } else {
